...
const getCollectionMemoryIds = (
  ...
  const collectionMemories = Array.isArray(
    collection.memories,
  )
    ? collection.memories
    : Array.isArray(
...
const MemoriesHeader = React.memo(
            ...
            >
              ...
              <Text
                style={[
                  ...
                  timelineMonth &&
                    styles.timelineButtonTextActive,
                ]}
              >
                {timelineMonth
                  ? MONTHS[
...
function MemoriesScreen({
  ...
  const timelineSheetRef =
    useRef(null);

  const refreshMemoriesScreen =
    useCallback(
      async () => {
        await Promise.all([
          refreshMemories(),
  ...
  const advancedFilterCount =
    useMemo(() => {
      ...
      if (
        advancedFilters.favoriteOnly
      ) {
        count += 1;
      }

      if (
        advancedFilters.hasPhotos
      ) {
        count += 1;
      }
      ...
      const searchInChanged =
        currentSearchIn.length !==
          defaultSearchIn.length ||
        defaultSearchIn.some(
          (field) =>
            !currentSearchIn.includes(
              field,
            ),
        );
  ...
  const displayedMemories =
    useMemo(() => {
      ...
      if (
        ...
        const collectionMemoryIds =
          getCollectionMemoryIds(
            selectedCollection,
          );
        ...
        filtered =
          filtered.filter(
            (memory) => {
              ...
              return collectionMemoryIds.has(
                String(memoryId),
              );
            },
          );
  ...
  const handleApplyTimelineMonth =
    useCallback((month) => {
